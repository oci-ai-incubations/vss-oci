...
class ChatOpenAINoTokenRewrite(ChatOpenAI):
    ...
    def _default_params(self) -> dict[str, Any]:
        ...
        if "gpt-5" in self.model_name or re.match(r"^o\d", self.model_name):
...
class ChatOpenAITool(LLMTool):
    ...
    def update_tool(self, config, tools=None):
        ...
        if not ("gpt-5" in model or re.match(r"^o\d", model)):
        ...
        elif "gpt-5" in model or re.match(r"^o\d", model):
    ...
    def update(
        ...
        if top_p is not None and not (
            "gpt-5" in model or re.match(r"^o\d", model)
        ):
